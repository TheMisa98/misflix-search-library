...
Responder siempre en español al usuario en este repo, sin importar el idioma del mensaje o de los comentarios del codigo.
...
uv run main.py <command>             # run the CLI (e.g. uv run main.py search movies "query", search books "query")
...
When adding a new provider, prefer testing its HTML parsing against saved fixtures
(see `tests/fixtures/zona_leros/`) rather than live network calls. If the target site
turns out to be behind Cloudflare, reuse `infra/cloudflare.py` rather than reaching for
Playwright — it doesn't work against a Managed Challenge for the reasons above.

The package installs as a script (`project.scripts` in `pyproject.toml`, entry point
`misflix.cli.app:app`) built with `hatchling` — that's what `~/.local/bin/misflix`
runs.
